...
RSpec.describe WordsHelper, type: :helper do
  describe 'next_review' do
    context 'with future reviews' do
      it 'returns in how many days the next review will be performed' do
        word = create(:review).word
        expect(next_review(word)).to match 'It will be reviewed in 10 days'
      end
    end
    ...
    context 'when no reviews to do either in future or today' do
      it do
        expect(next_review(create(:word))).to match 'It will NOT be reviewed'
      end
    end
  end
end
